...
> 本文檔用於快速了解專案的完整設計與實現細節，如需修改請有依據的修改，當前專案同時在macOS和Windows上進行開發，請注意環境的差異。並且可能有些模型不在同一台電腦中所以看不到，請注意。
...
### 第六階段：多幣種擴展（進行中）
1. ✅ 支援任意幣種下載與訓練（BTCUSDT / ETHUSDT / SOLUSDT / WIFUSDT）
2. ✅ 新幣種自動探測上市日期
3. ⏳ 降低最大回撤（調整 `atr_stop_multiplier`、加 reward 回撤懲罰）
...
- **v0.9** (2026-03-07): 多幣種支援 + 訓練/回測環境對齊 + 數據下載修復
  ...
  - ✅ 若 `start_date` 早於代幣上市日（如 WIFUSDT 2023 年上市），自動調整起點並印出警告，不再連續失敗 10 次後中止
  ...
  - ℹ️ Random seed 只影響訓練（權重初始化、PPO 探索採樣）；推論時 `deterministic=True`，與 seed 無關
